...
window.dongyi = window.dj = window.castapp = window.ca =  {
	 ...
	 * @param {Object} succfn  成功的回调
	 */
	blur:function(elementData,succfn){
		elementData.addEventListener('blur',function(){
			succfn(this);
		});
	},
	/**
	 ...
	 * @param {Object} succfn  成功的回调
	 */
	input:function(elementData,succfn){
		...
		gM.plusReady(function(){
			plus.nativeUI.actionSheet( {
			    ...
			    function(e){    	
			        ...
			        }else if(arrayData[index].title == '从相册选择'){
				        	dongyi.album({
	...
	pay:function(json){
		gM.ready(function(){		
			...
			if(json.type=='alipay' || json.type=='wxpay'){
				if(json.type=='wxpay'){
				url = json.url+"?type="+json.type+"&log_id="+json.log_id+"&order_sn="+json.order_sn+"&total=";
				}else{
				url = json.url+"?type="+json.type+"&log_id="+json.log_id+"&order_sn="+json.order_sn+"&total=";
				}
			}else{
			...
			function shareMessage(s,ex){
				...
				if(jsonData.shareLink){
					...
					if(jsonData.shareImg){
						...
						msg.pictures = msg.thumb=[jsonData.shareImg];
					}
				}else{
					if(jsonData.shareImg){
						msg.pictures = msg.thumb=[jsonData.shareImg];
				...
				//{content:"DCloud HBuilder-做最好的HTML5开发工具",href:"http://www.dcloud.io/",thumbs:["http://www.quhaixuan.com/images/201805/goods_img/903_G_1526726161261.jpg"],extra:{scene:ex}}
